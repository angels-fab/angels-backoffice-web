...
## 병렬 세션 규칙 (같은 폴더에서 Claude 세션 여러 개 동시 작업 시 — 항상 적용)

여러 세션이 **같은 작업 폴더·같은 git**을 공유하므로, 코드가 안 겹쳐도 git 조작이 겹치면 서로의 작업을 망가뜨릴 수 있다.

1. **작업 영역 분리** — 세션마다 맡은 파일 경로가 겹치지 않게 한다. 작업 시작 전 `git status`로 다른 세션의 수정 중 파일을 확인하고, 내 작업과 겹치면 진행하지 말고 사용자에게 보고한다.
2. **스테이징은 경로 지정만** — `git add -A`·`git add .` **금지**. 반드시 자기 작업 파일만 콕 집어 `git add <경로>`. (다른 세션의 미커밋 변경을 쓸어담는 유일한 사고 경로)
3. **푸시는 한 번에 한 세션만** — 다른 세션 활동 징후(3600 점유, 방금 생긴 낯선 수정 파일)가 있으면 **커밋까지만** 하고 푸시는 보고 후 사용자 지시를 기다린다. 푸시하면 로컬의 모든 커밋(다른 세션 것 포함)이 함께 올라간다는 것을 유의. **푸시 직전 `git pull --ff-only`로 원격 최신을 먼저 받는다** — 성공하면 안전하게 올리고, 거부되며 멈추면(그새 다른 세션이 올림) 억지로 진행하지 말고 사용자에게 보고한다(reset·merge로 임의 해결 금지 — 규칙 7).
4. **공유 파일은 자기 섹션만 추가** — `CLAUDE.md`·`docs/HANDOFF.md`·`index.css`·`src/constants/nav.tsx` 등 공용 파일은 최소 수정·추가 위주로. 구조 대개편은 한 세션이 전담한다.
5. **dev 서버** — 3600이 점유돼 있으면 그 서버를 끄지 말고 `.claude/launch.json`에 보조 포트 항목(예: 3610)을 추가해 자기 서버를 띄운다.
6. **DB 변경 선언** — Supabase 마이그레이션·같은 테이블 데이터 조작은 두 세션이 동시에 하지 않는다. DB를 만지기 전 HANDOFF나 사용자를 통해 선언하고 한 세션만.
7. **브랜치 되감기 금지** — `git reset`(soft 포함)·`--amend`로 HEAD를 옮기지 않는다. 그 사이 다른 세션이 얹은 커밋이 이력에서 떨어져 나간다(실제 사고 2026-07-23: v3 커밋 유실 후 복구). 고칠 게 있으면 항상 **새 커밋**으로.
...
- `src/pages/` — Home(대시보드), Notice, Calendar, Work, Equipment, Links, Milestone(팹 구축~개소 실행계획 현황판 — Supabase `milestones` 62건, 상태 4종 수동+임박·지연 자동파생, 퍼지 분기 매핑은 model.tsx·시딩 스크립트 주석 참조), Resources(자료실 — 링크·정보 카드, 개선요청 86, 중요 문서는 NAS라 업로드 없음)
