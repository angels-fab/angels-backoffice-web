...
- **카드 상태칩**: 진행중=초록 점멸 dot / 예정=노랑 dot+D-# / 종료=회색. **상세 팝업**: 포스터 전체 + 하단 그라데이션 위 정보. 팝업 칩은 포스터 위에서 **filled(색 배경 + 자동대비 글자, `getContrastText`)** 로 또렷하게(아웃라인은 어두운 배경서 흐림). **하단 '행사 사이트' 버튼만** `KIND_BLUE`(#3b82f6) 유지(분류색은 칩에만).
