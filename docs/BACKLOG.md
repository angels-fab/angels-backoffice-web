...
- **상태**: DB 마이그레이션 4개 적용 + GAS @64 배포(URL 불변). **남음**: ① 소유 계정으로 Apps Script 편집기에서 `authorizeSync` 1회 실행·승인(UrlFetchApp 신규 스코프) ② 토큰 주입+트리거 설치(承認 후 Claude가 실행) ③ 첫 동기화 E2E 검증(클레임 수·중복 점검, 7/13 연구개발특구 유입 확인).
