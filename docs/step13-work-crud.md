...
### 6-1. 완료일자 열 정리 (L열)
시트에서 중복돼 있던 완료일자(P열, 구 index 15)를 삭제하고 **L열(index 11) 완료일자만 유지**. 코드는 헤더명 매핑(`col(['완료일자','완료일'])`)이라 **변경 없이 L열을 사용** — getWorks로 완료일자 정상 수신 확인.

### 6-2. 검토 필요 자동 해제 (완료 시)
- 백엔드 `createWork_`/`updateWork_`: 상태='완료'면 `chief`(검토 필요)를 자동 `false`.
- `WorkWrite` 모달: 상태를 '완료'로 선택하면 검토 필요 체크를 해제하고 체크박스 비활성화("완료 시 자동 해제됨").

### 6-3. 정렬 — 발의일자 최신순
전체 업무 목록 정렬을 구분 우선순위 → **발의일자 기준 최신 업무가 위**로 변경(`cmp = b.start - a.start`).

### 6-4. 시트 직접 편집 자동화 (설치형 onEdit 트리거)
웹(updateWork)과 **동일 규칙을 구글시트 직접 편집에도 적용**:
- `onWorkStatusEdit(e)` — '센터 업무 현황' 시트의 **상태 열** 편집 감지(헤더명으로 열 인식):
  - 상태='완료' → 같은 행 **검토 필요 해제** + **완료일자에 오늘 날짜**(비어있을 때만; 직접 입력한 날짜는 유지)
  - 상태≠'완료' → **완료일자 비움**
- `setupWorkEditTrigger()` — 독립형 스크립트라 단순 onEdit가 안 떠서, 이 함수를 **Apps Script 편집기에서 1회 실행**해 설치형 onEdit 트리거를 등록(권한 동의 필요). 트리거는 함수명으로 연결되어 이후 코드 갱신은 자동 반영.

### 6-5. 결정사항(추가)
- 완료일자/검토필요 자동 규칙을 **웹·시트 양쪽 동일**하게 유지(일관성). 완료일자는 '상태=완료일 때만 유효'.
- 시트 트리거는 **상태 열 편집에만** 반응 → 완료일자/검토필요를 직접 수정하는 것은 방해하지 않음.

## 배포 이력
- 백엔드(clasp): @35(CRUD) → @36(검토필요 자동해제) → @37(시트 트리거) → **@38(시트 완료일자 자동입력)**. URL 불변.
- 프런트(git push → 자동배포): `20ad2a5`(CRUD) · `abcb547`(라벨/매핑) · `e5cb5bc`(자동해제·정렬) · `36db4be`(트리거) · `bfa1199`(완료일자 트리거).
- 사용자 1회 작업: Apps Script 편집기에서 `setupWorkEditTrigger` 실행(트리거 설치).
