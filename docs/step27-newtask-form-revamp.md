...
- **수정 인라인 카드 = 단일 칸(전폭 span 제거)**: `renderTask`의 편집 분기에서 `gridColumn:'1 / -1'` 래퍼 제거 → 일반 카드와 동일 폭(2열 그리드 한 칸). 작성(새 업무) 컴포저는 기존대로 전폭 유지.
- **Enter 자동 글머리 = 윗줄이 bullet일 때만**: `onBodyKeyDown` — 현재 줄이 `• ` 글머리면 같은 들여쓰기로 `• ` 이어쓰기, 빈 글머리 줄이면 글머리 제거, 그 외(평문·동그라미 줄)는 자동 글머리 없이 일반 줄바꿈.
- **'ㅇ'+숫자 → 동그라미 숫자(①~⑳)**: `onBodyChange`에서 줄 시작 `[ㅇᄋ](\d{1,2}) ` → `circledNumber(n)`. 입력 즉시 변환(공백 트리거), 커서 보정(`caretRef`).
- **동그라미 숫자 = 하위 글머리(들여쓰기)**: 변환 시 최소 2칸 들여쓰기 부여(상위 `• `의 하위 항목). 시트엔 들여쓴 `①` 그대로 저장(SubLine `[①-⑳]` 파싱이 들여쓰기 표시). `circledNumber()`는 `workMeta.ts`.

## 후속 보완 4 (사용자 피드백)

- **Enter 글머리 이어쓰기 삭제**: `onBodyKeyDown` 제거 → Enter는 일반 줄바꿈. '- '→'•', 'ㅇN '→동그라미 변환(입력 시)은 유지.
- **인라인 폼 필드 폭 슬림화**(내용 길이에 맞춤): 제목 210·담당자 74·발의일자/예정일 116·관련부서/장소 128·시간 150·구분 96. 본문 메타행을 grid→flex-wrap.
- **업무카드 더보기 메뉴**: 우측 수정/완료 아이콘 → 세로점(⋮) `MoreVert` 메뉴(완료·수정·삭제). 완료 상태 카드는 '완료' 항목 숨김. 삭제 추가(`onDelete`→`setDeleteTarget`).
- **수정 확인 팝업**: in-place 수정 '확인' → "수정하시겠습니까?" Dialog(`pendingEdit`→`confirmEdit`). **변경 사항 없으면 팝업 없이 닫기**(`isEditDirty`, 줄 끝 공백 무시 비교 `normTask`).
- **관련부서 칩 이동**: 본문 메타의 부서 → 제목줄 `구분칩` 다음 `info` 칩으로. 본문 메타는 예정/완료만.
- **발의일자 칩 높이 정렬**: 담당자/발의일자 칩 모두 `display:inline-flex; height:24; boxSizing:border-box`로 동일 높이.

## 후속 보완 5 (작성/수정 카드 레이아웃 확정 + 업무목록 헤더)

- **A 카드 레이아웃 확정(작성·수정 공용 `NewTaskCard`)**: 헤더 = `구분 · 관련부서 · 담당자` 칩 + 우측 링크/첨부/저장/취소. **제목은 헤더 안에서 전폭 한 줄**(`flexBasis:100%`). 메타 줄 = `발의일자 · 예정일 · 시간 · 장소`. (관련부서를 본문→헤더로, 발의일자를 헤더→메타로 이동)
- **B 업무목록 헤더 + 새 업무 카드 위치**: 진행중 뷰에서 그리드 위에 `업무 목록` 헤더(페이지 타이틀과 동일 `variant h2` 22px + 좌측 `Checklist` 아이콘을 PageHeader와 같은 40×40 라운드 박스로). 새 업무 `AddCard`를 2열 그리드 **좌상단 첫 칸**으로, 높이 150px 고정(`alignSelf:'start'`로 행에 안 끌려 늘어남). 작성 시에는 그 자리에서 컴포저가 전폭(`gridColumn 1/-1`)으로 펼쳐짐.
