...
| D3 | 상태 의미색 | **전역 배정표 확정** — 배정까지 사용자 지정 완료(2026-07-12): **진행중=그린 · 완료=파랑 · 예정=회색 · 보류=앰버 · 지연/불가=레드**. 보조 도메인 상태(도입중 teal 등)는 핵심 5의미와 충돌하지 않는 한 teal/purple 허용. 유의: 예정·종료가 공존하는 화면(Events)은 종료를 더 흐린 비활성 회색으로 구분. ⚠ 승인 포인트: 페이지별 적용 시안 + 색이 바뀌는 페이지 목록(로드맵 완료 그린→파랑·진행중 블루→그린 맞교환, Work 완료 회색→파랑 등) 제시 → 승인 후 적용 |
...
| D5 | 토큰 외 색 | **2단계** — ① 즉시 도메인 토큰 승격(`tokens.domain.*`: events·roadmap·manager·highlight·calendar, 화면 무변화) ② Events 3색·로드맵색의 accent 통합안을 A/B 시안으로 라이브 비교 후 통합 여부 결정. ⚠ 승인 포인트 |
...
10. **데이터표** — ds DataTable 신설(헤더·셀·모바일 규칙), 시안 확인 후 Improve·Notice 이관
