...
## 후속: `ㅇ1 ` 자동변환의 앞 공백 (사용자 확정 후 적용)

`ㅇ1 ` 입력 규칙(`RichContentEditor`의 `CircledNumRule`)은 `①` 앞에 공백 2칸을 함께 넣었다. 목록 항목 안에서는 목록 한 단계(18px) + 공백 2칸(18px) = 36px로, `①`를 직접 입력한 줄(18px)보다 깊어졌다.

→ **글머리 항목 안에서는 공백을 넣지 않도록** 수정(`state.doc.resolve(range.from)`의 조상에 `listItem`이 있는지 확인). 목록 밖에서는 공백 2칸 유지 — 거기선 공백이 유일한 들여쓰기 신호다.

헤드리스 실측: 항목 안 → `"① "`(공백 0), 목록 밖 → `"  ① "`(공백 2). 기존에 저장된 글은 그대로다(입력 시점 규칙이라 소급 안 됨).
