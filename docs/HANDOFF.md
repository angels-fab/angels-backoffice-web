...
> 갱신: 2026-07-07 (집) · 다음 작업: 개선요청 검토중 잔여(#7 종료행사 참석자, #8 업무현황 탭 다중선택, #10 데모결과 메뉴, #11 업무일정 UX 잔여) / STEP22 phase2 등
> 이 파일은 머신 간 동기화되지 않는 Claude 로컬 메모리를 대신해, 다른 PC에서 맥락을 빠르게 잡기 위한 요약입니다.

## 현재 상태
- **공지사항 첨부파일(개선요청 #6, 프런트+백엔드)**: Supabase Storage 비공개 버킷 `notice-files`(20MB·MIME무제한, RLS: 읽기=authenticated·쓰기=is_admin) + `notices.attachments jsonb`. 작성/수정에 "파일 첨부"(즉시 업로드·세션 orphan 정리), 상세에 "첨부파일 N" 다운로드(서명URL·원본 파일명). 신규 `attachmentUI.tsx`(MIME별 MUI 아이콘). type-check 통과·익명 업로드 403(RLS) 확인. **관리자 업로드/다운로드 실경로는 배포 후 관리자 로그인 실측 필요.** 상세 `docs/notice-attachments.md`.
