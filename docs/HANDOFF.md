...
> 갱신: 2026-07-08 (집) · 다음 작업: 개선요청 검토중 잔여(#8 업무현황 탭 다중선택, #10 데모결과 메뉴, #11 업무일정 UX 잔여) / STEP22 phase2 등
> 이 파일은 머신 간 동기화되지 않는 Claude 로컬 메모리를 대신해, 다른 PC에서 맥락을 빠르게 잡기 위한 요약입니다.

## 현재 상태
- **공지 저장 멈춤 버그 수정(프런트, 2026-07-08)**: 첨부 있는 공지 수정→저장 시 스피너 무한 대기. **원인(멀티에이전트 RCA)**: supabase-js는 모든 요청 직전 `getSession()`으로 토큰을 읽는데, 액세스 토큰 만료 임박(~90초)이면 내부에서 **타임아웃 없는** 토큰 갱신(POST /token)을 하고, 사무실망 프록시가 이 소켓을 붙잡으면 PATCH가 아예 안 나가고 멈춤(서버 로그에 PATCH 없음으로 확인). 첨부 편집은 폼을 오래 열어둬 토큰이 갱신 구간에 들어가 첨부 공지에서 잘 재현. **navigator.locks 데드락 아님**(supabase-js 2.110 기본 lockless). **수정**: `src/api/notices.ts`에 `ensureSession()`(쓰기 전 `withTimeout(getSession)`) 추가 → add/update/delete/upload 앞에 호출, 갱신 스톨을 '세션 확인 응답 지연' 에러로 빠르게 전환. `src/pages/Notice/index.tsx` 저장/삭제 핸들러 상태해제를 `finally`로 이동(멈춤 구조적 차단). **미검증**: 로그인 후 실측 필요(오피스망 재현). **후속**: events/works/improve 등 다른 write API도 같은 취약점 → ensureSession 전파 필요.
