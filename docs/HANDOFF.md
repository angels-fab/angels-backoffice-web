...
## 현재 상태
- **STEP 28 개선제안 페이지 — 보완 8 완료(백엔드 @52)**: ① **보류·불가 사유 저장/표시 버그 수정** — 시트 사유 헤더가 `사유 (보류/불가)`라 정확매칭 실패로 미저장/빈값이던 것을 `improveCtx_`에 '사유' 포함 폴백 추가로 해결. ② 정렬=번호 내림차순. ③ 0건 상태 탭 숨김. ④ Shift 다중선택 안내문. ⑤ **새 제안=표 바깥 우상단 버튼(폭≈표1/4) + 클릭 시 좌측 펼침 애니메이션**(inline-block width 0↔380 CSS transition, rtl로 우측앵커, 표 안 인라인행 제거). ⚠ flex+overflow:hidden 폭붕괴·sx maxWidth 0px 오계산·headless 프리뷰 transition 미진행 등 함정은 `docs/step28-improve-page.md` 보완8 참고.
