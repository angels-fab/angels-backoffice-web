...
/**
 * 흰 글자를 얹는 **솔리드 채움** (2026-08-02 토큰화 — 값은 index.css:58 에 있던 것 그대로).
 *
 * 왜 세 번째 계층이 필요한가:
 *   accent 원색(#5491DA)은 흰 글자 대비가 3.26:1 로 미달이고,
 *   theme.palette.primary.main 은 **accentText 다**(theme.ts:68 — 다크 #79AEF2 / 라이트 #225BB4).
 *   글자색으로 쓰라고 테마별로 밝기가 뒤집히게 만든 값이라, 이걸 **면**으로 쓰고 흰 글자를 얹으면
 *   다크에서 2.30:1 로 무너진다(실측). 채움 3계층이 각각 다른 일을 한다:
 *     accent      — 점·보더·프로그레스처럼 글자를 안 얹는 채움
 *     accentText  — 배경 위 **글자**
 *     solid       — 흰 글자를 얹는 **면**(배지·선택 알약·헤더 밴드). 테마 공통
 *
 * ★ 테마 공통인 것이 핵심이다. 면 위의 흰 글자는 테마가 바뀌어도 흰색이므로,
 *   면도 같이 밝아지면 안 된다. 새 solid 를 추가할 때도 흰 글자 4.5:1 을 반드시 확인할 것.
 */
export const solid = {
  /** 흰 글자 5.74:1 */
  blue: '#2563C4',
...
export const z = {
  ...
  sticky: 30,
  /**
   * 상단바 — 사이드바(100)보다 아래로 미끄러져 들어가되, **붙임쪽지 레이어(90)보다는 위**다.
   *
   * 2026-08-11(개선요청 63): 쪽지·그림이 문서에 붙어 콘텐츠와 함께 스크롤되면서, 위로 지나가는
   * 쪽지가 상단바를 덮고 메뉴·검색을 가렸다. 반대로 쪽지 층을 낮추면 그 안에 든 그리기 도구막대까지
   * 함께 가라앉는다 — 레이어가 쌓임 맥락을 만들어 안쪽 z 는 밖으로 못 나가기 때문이다.
   * 그래서 쪽지를 내리지 않고 **상단바를 올렸다**(하단 바 55·60 은 상단바와 겹칠 일이 없다).
   */
  topbar: 95,
...
export const typescale = {
  /**
   * 크롬 전용 최소 단 (2026-08-02 신설) — 배지 숫자·모바일 탭 라벨·간트 헤더처럼
   * "문장이 아닌 표시"에만. 본문·제목 금지.
   *
   * 왜 신설했나: 사다리 바닥(11)에 이름이 없어서 아래로 계속 새고 있었다 —
   * 9 · 9.5 · 10 · 10.5 로 18건. 그중 **8건이 서로 모르는 곳(배지·모바일 탭·간트)에서
   * 이미 10 에 수렴**해 있었다. 잡값이 아니라 이름 없는 단이었다는 뜻이라 이름을 준다.
   * 이 단은 사다리의 바닥이자 뚜껑이다 — 이 아래로 새 단을 만들지 말 것.
   *
   * ★ 크기만 쓰고 굵기는 weight.* 에서 따로 고른다. 실사용이 500·600·700 으로 갈려
   *   여기 weight 는 참고값일 뿐이다(BottomNav 500 · WhatsNewDialog 600 · NavBadge 700).
   */
  ...
  /**
   * 대표 수치 (2026-08-02 신설) — 화면에서 가장 큰 숫자 하나. KpiCard 가 유일한 소비처.
   *
   * display 를 두 단으로 나눈 이유: 실제 대표 수치가 28·34 두 크기로 쓰이는데 단이 하나뿐이라
   * 34 가 사다리 밖으로 밀려나 있었다. 34 를 28 로 흡수하면 홈 6장·마일스톤 5장의 숫자와
   * 아이콘이 함께 -6px 작아진다 — CLAUDE.md "작아 보이는 디자인 지양"과 충돌한다(사용자 결정 A안).
   *
   * 별도 "수치 전용 축"을 만들지 않고 같은 사다리 위쪽 밴드로 둔 것은 상용 시스템의 관행을 따른 것이다
   * (MD3 display L/M/S 는 headline 과 같은 스케일 위에 있고, Radix 도 큰 단을 같은 1~9 사다리에 둔다).
   * 축을 쪼개면 두 번째 사다리에서도 똑같이 잡값이 생겨 관리 대상만 늘어난다.
   */
  displayLg: { size: 34, weight: 800 },
} as const
...
export const iconSize = {
  ...
  header: 20,
  /**
   * 타일·설정 행 안을 채우는 아이콘 (2026-08-02 신설).
   * 24 = MUI 아이콘의 기본 크기(fontSize="medium") — 크기를 안 준 아이콘은 전부 이미 24로
   * 렌더된다. 천장을 20에 두면 "기본값이 사다리 밖"이 되어 24가 계속 재등장한다.
   *
   * ★ 이 위(30·34·38·44·48·84·92)는 일부러 비워 둔다. 사진 넘김 화살표·빈 상태 일러스트·
   *   카드 배경 워터마크로, 크기가 위계가 아니라 **컨테이너**에서 나오는 그래픽이다.
   *   사다리에 올리면 카드 규격을 바꿀 때마다 토큰을 고쳐야 한다.
   */
  feature: 24,
} as const
