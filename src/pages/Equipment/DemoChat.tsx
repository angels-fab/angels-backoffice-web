 ...
 * 띠 헤더(제목·작성자칩·날짜) + 본문. 발광 효과는 은은하게(할로·글자 글로우 최소).
 */
...
/**
 * 코멘트 — 비교표 아래, 제목 있는 메모카드 그리드(PC=여러 장, 모바일=1열). 네온 테마 고정.
...
export default function DemoChat({ memos, canPost, user, busy, onPost, onDelete }: {
  ...
  return (
    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))', gap: 1.25, alignItems: 'start' }}>
      {memos.map((m) => (
        <MemoCard key={m.id} m={m} own={!!user && m.author === user} onDelete={() => onDelete(m.id)} />
      ))}
