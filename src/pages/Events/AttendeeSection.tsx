    ...
    <Box sx={{ mb: 1.5, borderTop: 1, borderColor: 'divider', pt: 1.25 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1, mb: 0.8 }}>
        ...
        {!hideSelfToggle && isMember && user && onToggleSelf && (
          <Button size="small" variant={mine ? 'outlined' : 'contained'} color={mine ? 'inherit' : 'success'} disabled={busy} onClick={onToggleSelf} sx={{ fontSize: typescale.small.size, py: '2px', minWidth: 0, whiteSpace: 'nowrap' }}>
            {mine ? '참석 취소' : '내가 참석'}
          </Button>
        )}
      </Box>
      {rows.length > 0 ? (
        ...
        <Box sx={{ fontSize: typescale.small.size, color: 'text.disabled' }}>아직 참석자가 없습니다.</Box>
      )}
      {isAdmin && (
