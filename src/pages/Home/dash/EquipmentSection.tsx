 ...
 * 상태(시트 값 그대로): 도입예정 / 도입중 / 운영중 / 비가동. → /equipment 이동.
 */
export default function EquipmentSection() {
  ...
  return (
    <Box>
      ...
      <AppCard padding={18} sx={{ mb: 2 }}>
        <RatioBar
          segments={[
            ...
            { label: '도입중', value: u['도입중'], status: 'teal' },
      ...
      {/* 상태별 타일 */}
      <CardGrid columns={4}>
        <StatTile value={u['운영중']} unit="대" label="운영중" status="success" sub={`${eq.typesBy['운영중']}종`} onClick={go} />
        <StatTile value={u['도입중']} unit="대" label="도입중" status="teal" sub={`${eq.typesBy['도입중']}종`} onClick={go} />
