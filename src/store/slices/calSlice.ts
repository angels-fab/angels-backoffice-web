...
function expandRawEvent(ev: { id: string; title: string; start: string; end: string; allDay: boolean; loc: string; recurring: boolean; seriesId?: string; createdBy?: string }): CalEvent[] {
  const out: CalEvent[] = []
    ...
    moveCalEvent(state, action: { payload: { id: string; start: string; end: string } }) {
      ...
      state.events = state.events
        ...
        .concat(expandRawEvent({
          id, title: first.title, loc: first.loc, allDay: first.allDay, recurring: first.recurring, seriesId: first.seriesId,
          createdBy: first.createdBy, start, end,
        }))
    },
  },
