...
- **STEP24(업무현황 회의 뷰, 미커밋)**: /work 개편 — 업무목록을 KPI 바로 아래로, 기본 진행중, 진행중=아코디언(모두 펼침·개별 접기, 신규 `Work/TaskAccordion.tsx`), RatioBar 제거, '긴급 업무'→'Remind', 담당자현황 숨김(`SHOW_MANAGER_STATUS=false`, 코드 보존), 검토필요→'검토'. 프런트 전용. type-check+build 통과, 라이브 검증 + 적대적 리뷰(9건 중 3확정: TaskCard 라벨·아코디언 장소중복 수정, urgent 변수명은 지시대로 미수정).
...
- Codex: STEP23·24 + 상태 어휘 개편 **커밋·자동배포 완료**. 검토 후 다음 프롬프트(STEP22 phase2 / 담당자현황 재노출 형태 / 발표모드 등) 작성. **운영 규칙 변경**: 사용자 지시로 **작업 완료 시 자동 커밋·푸시(자동배포)** — 더 이상 '미커밋 대기' 안 함. **잔여**: 시트 '가동중'→'운영중' 마이그레이션. 백엔드 무변경(@42).
